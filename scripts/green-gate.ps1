...
# Give kube a moment to finish terminating old pods
...
function Is-ReadyPod($p) {
  if ($p.metadata.deletionTimestamp) { return $false }   # terminating
  ...
  foreach ($cs in $p.status.containerStatuses) {
    if (-not $cs.ready) { return $false }
  }
  return $true
}

$readyPods = @($podsJson.items | Where-Object { Is-ReadyPod # scripts/green-gate.ps1
# Green Gate (PS5.1-safe, parser-safe):
#  - Truth Gate (compile/tests/guards)
#  - Build+push ECR image from HEAD (or validate ImageTagOverride exists)
#  - Helm deploy to css-mock
...
if ($readyPods.Count -lt 1) {
  throw "No Running+Ready pods found yet for selector: $PodSelector"
}
...
if ($bad.Count -gt 0) {
  $bad | ForEach-Object { Write-Host # scripts/green-gate.ps1
# Green Gate (PS5.1-safe, parser-safe):
#  - Truth Gate (compile/tests/guards)
#  - Build+push ECR image from HEAD (or validate ImageTagOverride exists)
#  - Helm deploy to css-mock
