...
.PARAMETER SkipEcrCheck
  Skip the ECR â€œtag existsâ€ preflight check (not recommended)
...
# IMPORTANT: This file is maintained by humans. This script MUST NOT rewrite it.
# The only allowed mutation is pinning image.tag for this deploy.

# Update only image.tag in the pinned override file
...
Write-Host "DEPLOY OK" -ForegroundColor Green
  ...
  # Replace existing tag line inside image block. If tag is missing, insert it under repository.
...
, ('  tag: "' + $ImageTag + '"'))
    ...
    # Insert after repository if present, else directly after image:
...
' + "`n" + '  tag: "' + $ImageTag + '"'))
  ...
  Set-Content -Path $ValuesPath -Value $raw -Encoding UTF8
}

# Use pinned override, but do NOT rewrite it
