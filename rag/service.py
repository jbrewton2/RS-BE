...
def _norm_section_id_for_pack(v: Any) -> str:
    """
    Normalize section identifiers coming from section_question_map() so they match
    SECTION_QUERY_PACKS keys (slug ids like "mission-objective").
    """
    try:
...
# SECTION_FINDINGS_RANKING_V1
# SECTION_FINDINGS_RANKING_V2
def _derive_tier(r: Dict[str, Any]) -> str:
    """
    Map current risk schema -> tier label.
    Priority: deterministic flags > heuristics > section-derived > inference(ai_only)
    """
    if not isinstance(r, dict):
        return "tier1"

    # explicit tier already present
    t = r.get("tier")
    try:
        ...
        ts = ""
    if ts:
        if ts in ("tier3","tier_3","t3","3"): return "tier3"
        if ts in ("tier2","tier_2","t2","2"): return "tier2"
        if ts in ("tier1","tier_1","t1","1"): return "tier1"

    # infer from source/category
    try:
        src = str(r.get("source") or "").strip().lower()
    except Exception:
        src = ""
    try:
        cat = str(r.get("category") or r.get("risk_category") or r.get("riskCategory") or "").strip().lower()
    except Exception:
        cat = ""

    blob = (src + " " + cat).strip()

    if ("flag" in blob) or ("autoflag" in blob) or ("determin" in blob):
        return "tier3"
    if ("heur" in blob) or ("taxonomy" in blob):
        return "tier2"
    if ("section" in blob):
        return "tier2"
    # ai_only inference
...
def _norm_confidence(r: Dict[str, Any]) -> str:
    """
    Normalize confidence into High/Medium/Low strings.
    Supports numeric confidence (0..1) and string.
    """
    if not isinstance(r, dict):
        return ""
    c = r.get("confidence")
...
def _risk_tier_rank(v: Any) -> int:
    # lower is better
    try:
        s = str(v or "").strip().lower()
    except Exception:
        return 99
    if s in ("tier3", "tier_3", "3", "t3"): return 0
    if s in ("tier2", "tier_2", "2", "t2"): return 1
    if s in ("tier1", "tier_1", "1", "t1"): return 2
    # Some risks may store tier as int
    try:
        i = int(v)
        if i == 3: return 0
        if i == 2: return 1
...
def _norm_section_slug(v: Any) -> str:
    try:
        s = str(v or "").strip().lower()
    except Exception:
        return ""
...
def _route_risk_section_id(r: Dict[str, Any]) -> str:
    """
    Deterministic routing:
    1) honor explicit section id if present (try multiple known keys)
    2) map by category/label keywords
    3) fallback: route to OVERVIEW (so findings never empty when risks exist)
    """
    if not isinstance(r, dict):
        return ""

    # 1) explicit section-ish fields (expanded set)
    for k in (
        "section_id","sectionId","section","section_key","sectionKey",
        "section_slug","sectionSlug","section_name","sectionName",
        "source_section","sourceSection","derived_section","derivedSection",
        "section_derived_from","sectionDerivedFrom","section_id_norm"
    ):
        try:
            v = r.get(k)
        except Exception:
            v = None
        sid = _norm_section_slug(v)
        if sid:
            return sid

    # 2) category / label routing (best-effort; include more possible field names)
    try:
        cat = str(
            r.get("category")
            or r.get("risk_category")
            or r.get("riskCategory")
            or r.get("type")
            or r.get("risk_type")
            or r.get("riskType")
            or ""
    ...
    if _has("dfars","cmmc","rmf","ato","il5","il4","srg","encryption","logging","audit","stig","vuln","sbom","hosting","opensearch","bedrock"):
        return "security-compliance-hosting-constraints"
    if _has("data rights","unlimited rights","restricted rights","intellectual","ip","license","indemn","nda","flowdown"):
        return "legal-data-rights-risks"
    if _has("clin","pricing","invoice","payment","funding","ceiling","cpff","ffp","t&m","allowable","overrun","burn rate"):
        return "financial-risks"
    if _has("cdrl","deliverable","drl","did","milestone","schedule","ims","no later than","due date","timeline"):
        return "deliverables-timelines"
    if _has("clearance","citizen","citizenship","cac","piv","background","key personnel","labor category"):
        return "eligibility-personnel-constraints"
    if _has("section l","proposal","submission","portal","deadline","page limit","format","instructions to offerors"):
        return "submission-instructions-deadlines"
    if _has("conflict","contradict","order of precedence","inconsisten","ambiguous"):
        return "contradictions-inconsistencies"

    # 3) fallback route
    return "overview"

def _risk_rank_key(r: Dict[str, Any]) -> Any:
    """
    Sort key for selecting display findings:
      tier (t3 best) -> severity -> confidence -> evidence presence
    """
    if not isinstance(r, dict):
        return (99, 99, 99, 99)

    tier = _risk_tier_rank(_derive_tier(r))
    sev  = _sev_rank(r.get("severity"))
    conf = _conf_rank(_norm_confidence(r))

    # evidence strength: prefer risks with explicit evidence ids/attachments
    has_ev = 0
    try:
        ev = r.get("evidence") or r.get("evidence_ids") or r.get("evidenceIds") or []
        if isinstance(ev, list) and len(ev) > 0:
            has_ev = 1
    except Exception:
        has_ev = 0

    return (tier, sev, conf, 0 if has_ev else 1)

def se_populate_section_findings(
    sections: List[Dict[str, Any]],
    risks: List[Dict[str, Any]],
    *,
    max_findings_per_section: int = 6,
) -> List[Dict[str, Any]]:
    """
    Add `risk_findings` to each section based on deterministic routing + ranking.

    CRITICAL:
      - DO NOT overwrite `findings` because `se_parse_sections()` uses it for
        section narrative bullets / parsed content (strings).
      - `risk_findings` is the risk-focused list intended for UI display.
    """
    ...
    # bucket risks by routed section_id
    by_sid: Dict[str, List[Dict[str, Any]]] = {}
    for r in risks:
        if not isinstance(r, dict):
            continue
        sid = _route_risk_section_id(r)
        if not sid:
            continue
    ...
    for s in sections:
        if not isinstance(s, dict):
            continue
        sid = _norm_section_slug(s.get("id") or "")
        if not sid:
            continue

        cand = by_sid.get(sid, []) or []
        cand_sorted = sorted(cand, key=_risk_rank_key)

        out: List[Dict[str, Any]] = []
        for r in cand_sorted[: max(0, int(max_findings_per_section))]:
            try:
                out.append(
                    {
                        "tier": _derive_tier(r),
                        "severity": (r.get("severity") or ""),
                        "confidence": _norm_confidence(r),
                        "category": (r.get("category") or r.get("risk_category") or r.get("riskCategory") or ""),
                        "label": (r.get("label") or r.get("title") or r.get("name") or ""),
                        "why": (r.get("why") or r.get("rationale") or r.get("details") or ""),
                        "source": (r.get("source") or ""),
                        "evidence": (r.get("evidence") or r.get("evidenceIds") or r.get("evidence_ids") or []),
                    }
                )
            except Exception:
                continue

        # Always set risk_findings (even empty) for UI contract
        s["risk_findings"] = out

        # risk_level derived from top finding severity
        try:
            if out:
                s["risk_level"] = str(out[0].get("severity") or "").strip() or "Unknown"
            else:
                s["risk_level"] = "None"
        except Exception:
...
def rag_analyze_review(
    *,
    ...
    try:
            ...
            try:
                for _sid, _q in (section_question_map or []):
                    if not _q:
                        continue
                    retrieved[str(_q)] = (retrieved_by_section or {}).get(_norm_section_id_for_pack(_sid), []) or []
            except Exception:
                pass
    ...
    if (not force_reingest) and (not auto_reingest_used) and (docs or []) and ("retrieval_failed" not in warnings):
        _rt = 0
        try:
            _src = retrieved_by_section if isinstance(retrieved_by_section, dict) and retrieved_by_section else (retrieved or {})
            for _hits in (_src or {}).values():
    ...
    # SECTION_FINDINGS_CALLSITE_V1: populate per-section risk_findings (tier/sev/conf ranked)
    try:
        _max_find = int((_env("RAG_SECTION_FINDINGS_MAX_PER_SECTION", "6") or "6").strip() or "6")
    except Exception:
        _max_find = 6
    try:
        sections = se_populate_section_findings(sections, risks, max_findings_per_section=int(_max_find))
        # MARKER: risk_findings callsite executed
        _rf_added = 0
        try:
            for _s in (sections or []):
                if isinstance(_s, dict) and ('risk_findings' in _s) and (_s.get('risk_findings') is not None):
                    _rf_added += 1
        except Exception:
            _rf_added = -1
        try:
            warnings.append('risk_findings_ok:' + str(int(_rf_added)))
        except Exception:
            pass

    except Exception as e:
        try:
            warnings.append("risk_findings_failed:" + repr(e))
        except Exception:
            pass
